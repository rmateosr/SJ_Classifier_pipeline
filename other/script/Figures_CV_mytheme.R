...
ggsave(
  plot = ggbx,
  file = paste0("output/figures/Barplot_Scores_CV.png"),
 # height = 10,
  dpi=100, width = 20, height=10)
...
box = ggplot(resultadosnumericPANCANCERforplot, aes(x = `Mutation labels` , y =  score, fill = `Mutation labels`)) + geom_boxplot( color = "black")+ scale_fill_manual(values = myColors, drop = FALSE) + scale_color_manual(values = myColors, drop = FALSE)+ theme_minimal()  +
  geom_hline(yintercept=10, linetype="dashed", color = "red") + theme(text = element_text(size = 25))  +my_theme()
#dev.off()
ggsave(
  plot = box,
  file = paste0("output/figures/Boxplot_Scores_CV.png"),
  # height = 10,
   dpi=100, width = 20, height=10)
...
ggbx1 = ggbx1 + scale_fill_manual(values = myColors, drop = FALSE) + scale_color_manual(values = myColors, drop = FALSE)+ theme_minimal()  + geom_hline(yintercept=10, linetype="dashed", color = "red")+
  theme(axis.text.x=element_blank(), #remove x axis labels
        axis.ticks.x=element_blank(),text = element_text(size = 20)) + my_theme()qsub -N Figures_CV -cwd -j yes -l s_vmem=64G -hold_jid Step11_CV script/Figures_CV.sh $metadata_path
...
ggsave(
  plot = ggbx1,
  file = paste0("output/figures/Barplot_Scores_CV_mutated.png"),
  # height = 10,
   dpi=100, width = 20, height=10)
